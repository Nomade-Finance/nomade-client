...
import { ApiData, FormData } from '@/lib/types/index'
import React, { useCallback, useEffect, useState } from 'react'
import { getBinanceData, getExchangeRateData } from '@/app/api/routes'

import AutorenewIcon from '@mui/icons-material/Autorenew'
import { BinanceResponse } from "@/lib/interfaces/interfaces";
import Button from '@/components/Button/Button'
import { ListePaiement } from '@/lib/data/paymentsList'
import { ListeToken } from '@/lib/data/tokensList'
...
function Conversion () {
  // Constantes
  const TRANSACTION_FEE = parseFloat(process.env.NEXT_PUBLIC_TRANSACTION_FEE || '0') // Frais de transaction 5%
  ...
  const [cryptoData] = useState<BinanceResponse[]>([]) // Données des crypto-monnaies
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await getExchangeRateData() // Récupérer le taux de change depuis l'API
      const rate = response.rates.XOF;
      const bitcoinResponse = await getBinanceData('BTC') // Récupérer le taux de change du Bitcoin depuis l'API
      const bitcoinRate = Number(bitcoinResponse.price);
      const usdcResponse = await getBinanceData('USDC') // Récupérer le taux de change de l'USDC depuis l'API
      const usdcRate = Number(usdcResponse.price);
